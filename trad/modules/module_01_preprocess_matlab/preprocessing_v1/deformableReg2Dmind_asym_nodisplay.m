...
function [u1,v1,deformed]=deformableReg2Dmind_asym_nodisplay(im1,im2,alpha,flag2)
...
if nargin<4
    flag2=false;
    %true would disable re-calculation of MIND descriptors in each iteration
end
...
for j=1:length(levels)
    ...
    %filter kernel for image smoothing (currently disabled)
    ...
    for i=1:maxwarp
        ...
        % solve Euler-Lagrange equations using MATLAB's solvers
        [u1,v1,tOp1]=solveFlowMldivide(Sx,Sy,S,u1,v1,alpha);
...
Sy=imfilter(mind2,deriv','replicate');
...
function [u1,v1,tOp]=solveFlowMldivide(Ix,Iy,It,u0,v0,alpha)
[m,n,o]=size(Ix);

%laplace operator
x1=ones(m,n).*alpha; x1(1,:)=0;
x2=ones(m,n).*alpha; x2(end,:)=0;
y=ones(m,n).*alpha;
