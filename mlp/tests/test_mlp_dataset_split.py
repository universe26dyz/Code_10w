...
def test_generated_h5_splits_are_disjoint_by_timing_id(tmp_path):
    ...
    np.savez_compressed(pool, timing9_ms=np.asarray([[60] * 9, [70] * 9, [80] * 9], dtype=np.float64), tr_ms=np.asarray(3.2), vps=np.asarray(32), source_id=np.asarray(["a", "b", "c"]), group_id=np.asarray([0, 1, 2], dtype=np.int64), stack_idx=np.asarray([0, 0, 0], dtype=np.int64))
    result = generate_mlp_dataset(pool, "configs/protocol_hhz_v1.yaml", tmp_path / "h5", {"train": 8, "valid": 4, "test": 4}, seed=3, chunk_size=4)
    sets = []
