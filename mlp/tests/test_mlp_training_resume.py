...
def _train(tmp_path, epochs, *, output_name="run", resume=None):
    dataset = _dataset(tmp_path) if not (tmp_path / "rr").exists() else tmp_path / "rr"
    output = tmp_path / output_name
    return train_module.train_mlp(dataset, _config(epochs), "mlp/configs/protocol_hhz_v1.yaml", output, resume=resume), dataset, output
