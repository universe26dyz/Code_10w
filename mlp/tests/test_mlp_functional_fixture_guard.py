...
def test_functional_checkpoint_requires_explicit_smoke_allowance(tmp_path):
    ...
    with pytest.raises(ValueError, match="functional-fixture"):
        load_frozen_mlp_decoder(path, dataset, "configs/protocol_hhz_v1.yaml", allow_functional_fixture=False, device=torch.device("cpu"))
    decoder = load_frozen_mlp_decoder(path, dataset, "configs/protocol_hhz_v1.yaml", allow_functional_fixture=True, device=torch.device("cpu"))
