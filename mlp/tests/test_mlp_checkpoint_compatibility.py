...
def test_checkpoint_loader_rejects_online_timing_outside_training_range(tmp_path):
    ...
    with pytest.raises(ValueError, match="outside the MLP training timing domain.*timing2"):
        load_frozen_mlp_decoder(checkpoint, dataset, "mlp/configs/protocol_hhz_v1.yaml", allow_functional_fixture=True, device=torch.device("cpu"))
...
def test_checkpoint_loader_accepts_float32_rounding_at_a_stored_timing_boundary(tmp_path):
    ...
    decoder = load_frozen_mlp_decoder(checkpoint, dataset, "mlp/configs/protocol_hhz_v1.yaml", allow_functional_fixture=True, device=torch.device("cpu"))
