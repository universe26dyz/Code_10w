...
def test_benchmark_writes_separate_method_peak_fields_and_speedups(tmp_path):
    ...
    data = torch.load(checkpoint, weights_only=False); data.update({"formal_candidate": False, "validation_status": "functional_smoke"}); torch.save(data, checkpoint)
    pool = tmp_path / "pool.npz"
    np.savez_compressed(pool, timing9_ms=np.asarray([[60.0] * 9, [70.0] * 9, [80.0] * 9]), tr_ms=np.asarray(3.2), vps=np.asarray(32), source_id=np.asarray(["a", "b", "c"]), group_id=np.arange(3), stack_idx=np.zeros(3, dtype=np.int64), subject_id=np.asarray(["S1", "S2", "S3"]), functional_fixture=np.asarray(True))
