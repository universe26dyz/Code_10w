...
def test_h5_target_is_the_normalized_hhz_trad_teacher_signal(tmp_path):
    ...
    np.savez_compressed(pool, timing9_ms=np.asarray([[60] * 9, [70] * 9, [80] * 9], dtype=np.float64), tr_ms=np.asarray(3.2), vps=np.asarray(32), source_id=np.asarray(["a", "b", "c"]), group_id=np.asarray([0, 1, 2]), stack_idx=np.asarray([0, 0, 0]))
    generate_mlp_dataset(pool, "configs/protocol_hhz_v1.yaml", tmp_path / "h5", {"train": 4, "valid": 2, "test": 2}, seed=4, chunk_size=2)
