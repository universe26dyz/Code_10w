"""Fair, decoder-only Trad-versus-frozen-MLP benchmark on RR synthetic timings."""
...
from mlp.modules.module_05_signal_decoder.synthetic_dataset import protocol_from_record
from mlp.modules.module_05_signal_decoder.trad_teacher.trad_signal_simulator import TradSignalSimulator
...
def benchmark_signal_decoder(checkpoint_path: str | Path, rr_dataset_dir: str | Path, protocol_path: str | Path, output_path: str | Path, device_name: str, batch_sizes: list[int], repetitions: int = 3) -> list[dict[str, Any]]:
    ...
    rr_root = Path(rr_dataset_dir)
    meta = json.loads((rr_root / "dataset_metadata.json").read_text(encoding="utf-8"))
    if meta.get("schema") != "mlp_rr_synthetic/v1" or meta.get("split_mode") != "rhythm":
        raise ValueError("Benchmark requires the active mlp_rr_synthetic/v1 rhythm-disjoint dataset.")
    protocol = protocol_from_record(meta["protocol"], protocol_path)
    all_timing = np.asarray(np.load(rr_root / "rhythms.npz", allow_pickle=False)["timing9_ms"], dtype=np.float32)
    timing = np.asarray(all_timing[0], dtype=np.float64)
    dataset = type("RRBenchmarkTiming", (), {"timing": torch.from_numpy(all_timing), "validated_tr_vps": lambda self: (protocol.tr_ms, protocol.vps)})()
...
def main() -> None:
    ...
    parser.add_argument("--checkpoint", required=True); parser.add_argument("--rr-dataset-dir", required=True); parser.add_argument("--protocol", required=True); parser.add_argument("--output", required=True); parser.add_argument("--device", required=True); parser.add_argument("--batch-size", type=int, action="append", required=True); parser.add_argument("--repetitions", type=int, default=3)
    args = parser.parse_args()
    print(json.dumps(benchmark_signal_decoder(args.checkpoint, args.rr_dataset_dir, args.protocol, args.output, args.device, args.batch_size, args.repetitions), indent=2))
