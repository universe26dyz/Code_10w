...
def _teacher_signal(teacher: TradSignalSimulator, t1: torch.Tensor, t2: torch.Tensor, b1: torch.Tensor, timing: torch.Tensor, protocol: TradProtocol) -> torch.Tensor:
    """Signal-only teacher path: intentionally avoids autograd/Jacobian work."""
...
def _generate_rr_split(path: Path, rhythm_ids: np.ndarray, rhythms: Mapping[str, np.ndarray], *, tissue_samples_per_rhythm: int, seed: int, chunk_size: int, device: torch.device, store_jacobian: bool) -> int:
    ...
    with h5py.File(path, "w") as handle:
        ...
        for start in range(0, n_samples, chunk_size):
            end = min(start + chunk_size, n_samples); ids = expanded_ids[start:end]; count = end - start
            ...
            target_set[start:end] = target.cpu().numpy().astype(np.float32); rhythm_set[start:end] = ids
    return n_samples
...
def generate_rr_synthetic_dataset(output_dir: str | Path, settings: Mapping[str, Any]) -> dict[str, Any]:
    ...
    protocol = _fixed_rr_protocol(); split_counts = settings["split_rhythms"]
    if not isinstance(split_counts, Mapping):
        raise ValueError("split_rhythms must be a train/valid/test mapping.")
    config = RRDomainConfig(n_rhythms=int(settings["n_rhythms"]), seed=int(settings["seed"]), tr_ms=protocol.tr_ms, vps=protocol.vps, ti_ms=tuple(protocol.ti_ms), t2prep_ms=tuple(protocol.t2prep_ms))
    rhythms = generate_rhythm_timing9(config); splits = split_rhythm_ids(config.n_rhythms, seed=config.seed, counts={key: int(value) for key, value in split_counts.items()})
    output = Path(output_dir)
    if output.exists() and any(output.iterdir()):
        raise FileExistsError(f"RR synthetic output must be absent or empty: {output}")
    output.mkdir(parents=True, exist_ok=True); device = _teacher_device(str(settings["teacher_device"]))
    sizes = {name: _generate_rr_split(output / f"{name}.h5", ids, rhythms, tissue_samples_per_rhythm=int(settings["tissue_samples_per_rhythm"]), seed=config.seed + offset, chunk_size=int(settings["teacher_chunk_size"]), device=device, store_jacobian=bool(settings["store_jacobian"])) for offset, (name, ids) in enumerate(splits.items())}
    ...
    metadata = {"schema": "mlp_rr_synthetic/v1", "split_mode": "rhythm", "timing_pool_sha256": None, "store_jacobian": bool(settings["store_jacobian"]), "rhythm_split_ids": {name: ids.tolist() for name, ids in splits.items()}, "sizes": sizes, "protocol": {"tr_ms": protocol.tr_ms, "vps": protocol.vps, "fa_deg": list(protocol.fa_deg), "ti_ms": list(protocol.ti_ms), "t2prep_ms": list(protocol.t2prep_ms), "n_ramp_up": protocol.n_ramp_up}, "rr_domain": {"hr_mean_bpm": [20.0, 120.0], "rr_cv": [0.0, 0.50], "generator": "mDM-inspired coherent normal RR histories; not byte-for-byte upstream mDM"}, "parameter_ranges": {"t1_ms": [20, 2500], "t2_ms": [5, 200], "b1": [0.1, 1.2], "constraint": "T1>T2"}, "timing9_min_ms": timing_min, "timing9_max_ms": timing_max, "train_timing9_min_ms": timing_min, "train_timing9_max_ms": timing_max, "pool_timing9_min_ms": timing_min, "pool_timing9_max_ms": timing_max, "train_subject_ids": [], "valid_subject_ids": [], "test_subject_ids": [], "functional_fixture": False, "formal_mlp_split": False, "test_timing_relation_to_train": "synthetic_rhythm_in_domain", "seed": config.seed, "teacher_device": str(device)}
