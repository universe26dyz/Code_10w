...
from .synthetic_dataset import load_timing_pool
from .test_mlp import fidelity_metrics
from .train_mlp import load_h5_split, validate_dataset_provenance


def validate_formal_mlp(checkpoint_path: str | Path, dataset_dir: str | Path, timing_pool: str | Path, protocol: str | Path, output_path: str | Path, device_name: str, batch_size: int, gradient_samples: int) -> dict[str, object]:
    output_path = Path(output_path)
    if output_path.exists(): raise FileExistsError(f"Formal validation output already exists: {output_path}")
    metadata, pool, _ = validate_dataset_provenance(dataset_dir, timing_pool, protocol)
    ...
    if checkpoint.get("timing_pool_sha256") != metadata["timing_pool_sha256"]: raise ValueError("Checkpoint and dataset timing-pool SHA256 differ.")
    model = MdmSignalMLP().to(device_name); model.load_state_dict(checkpoint["state_dict"]); model.eval()
    metrics = fidelity_metrics(model, load_h5_split(Path(dataset_dir) / "test.h5", "test"), pool, str(protocol), batch_size, gradient_samples)
...
def main() -> None:
    ...
    for name in ("checkpoint", "dataset-dir", "timing-pool", "protocol", "output", "device"): parser.add_argument(f"--{name}", required=True)
