"""Run one explicit three-stack MLP reconstruction from prepared observations."""
...
def reconstruct_subject(prepared_root: str | Path, subject_id: str, config: str | Path, output: str | Path, mlp_checkpoint: str | Path) -> dict[str, object]:
    ...
    return run_reconstruction(config, method_root / "configs" / "protocol_hhz_v1.yaml", observations, output, mlp_checkpoint=mlp_checkpoint, subject_id=subject_id)


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--prepared-root", required=True); parser.add_argument("--subject-id", required=True)
    parser.add_argument("--config", required=True); parser.add_argument("--output", required=True); parser.add_argument("--mlp-checkpoint", required=True)
    args = parser.parse_args()
    print(json.dumps(reconstruct_subject(args.prepared_root, args.subject_id, args.config, args.output, args.mlp_checkpoint), sort_keys=True))
