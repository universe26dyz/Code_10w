...
def approve_formal_checkpoint(checkpoint_path: str | Path, validation_report: str | Path, approved_output: str | Path, review_note: str) -> dict[str, object]:
    checkpoint_path, validation_report, approved_output = Path(checkpoint_path), Path(validation_report), Path(approved_output)
    ...
    return {"approved_output": str(approved_output), "validation_status": approved["validation_status"], "source_checkpoint_sha256": source_sha}
...
def main() -> None:
    ...
    print(json.dumps(approve_formal_checkpoint(args.checkpoint, args.validation_report, args.approved_output, args.review_note), ensure_ascii=False, indent=2))
