...
SMOKE_ROOT="/tmp/multimap_phase5_mlp_smoke"
REAL_POOL="${SMOKE_ROOT}/timing_pool_real.npz"
FIXTURE_POOL="${SMOKE_ROOT}/timing_pool_functional.npz"
DATASET_DIR="${SMOKE_ROOT}/synthetic_h5"
...
if [[ ! -f "${PREPARED_NPZ}" ]]; then
  echo "Required existing Phase-4 tiny prepared NPZ is unavailable; do not rerun MIND/MP-PCA." >&2
  exit 1
fi
mkdir -p "${SMOKE_ROOT}"
cd "${PROJECT_ROOT}"
conda run -n knesvr_torch python -m modules.module_05_signal_decoder.build_timing_pool \
  --observations "${PREPARED_NPZ}" --output "${REAL_POOL}"
...
conda run -n knesvr_torch python -m modules.module_05_signal_decoder.generate_mlp_dataset \
  --config configs/mlp_smoke_cpu.yaml --timing-pool "${FIXTURE_POOL}" \
  --protocol configs/protocol_hhz_v1.yaml --output-dir "${DATASET_DIR}"
conda run -n knesvr_torch python -m modules.module_05_signal_decoder.train_mlp \
  --config configs/mlp_smoke_cpu.yaml --dataset-dir "${DATASET_DIR}" \
  --timing-pool "${FIXTURE_POOL}" --protocol configs/protocol_hhz_v1.yaml --output-dir "${OUTPUT_DIR}"
